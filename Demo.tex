%!TEX program = xelatex
\documentclass[WordOneHalf,print,mac,adobe]{XDUthesis}   %%启用默认字体设置，word 1.5 倍行距，打印样式
% 参数设置：
%     功能设置：
...
\title{基于springboot的视频实时监控系统的设计与实现}
\author{陈鑫辉}
\date{\qquad 年\qquad 月\qquad 日} %%%  请勿编辑这行
%题目拆分：用于封面
\septitleA{基于springboot的视频实时}%如果论文题目长度<=11中文字符只需填此项即可B项空着
\septitleB{监控系统的设计与实现}%如果论文题目长度>11中文字符, 建议拆分为10+x or 11+x 将剩余x个字符填在此处。
\class{1703014}%班级号
\schoolnumber{17039110002}%学号
\major{计算机科学与技术}%专业
\school{计算机科学与技术}%学院
\supervisor{鱼滨}%指导老师
