...
实时消息传输协议（Real-Time Messaging Protocol，RTMP） 最初是Macromedia公司为了满足在互联网上传输流媒体音视频而开发的一个私有协议。
RTMP是基于TCP的明文协议，端口的缺省值为1935\cite{周文琼2019智慧供电服务柜员机系统设计与实现}。
...
在数据发送时，消息会被分割为消息块，然后消息块会通过TCP发送出去\cite{国之重器出版工程：云化虚拟现实技术与应用}。
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=.8\linewidth]{./Figure/IMG_rtmp.pdf}
    \caption{消息拆分示意图}
    \label{Fig:rtmp}
\end{figure}
...
表 \ref{Tab:pcompare} 对RTMP、 HTTP-FLV、 HLS这三种协议在底层传输协议、视频封装格式、延时等方面进行了对比。
...
\label{Tab:pcompare}\\
\hline
协议&底层传输协议&视频封装格式&延时&数据分段&HTML5\\
\hline
RTMP&TCP&flv tag&2秒&连续流&不支持\\
\hline
HTTP-FLV&HTTP&flv&2秒&连续流&支持\\
...
HTTP-FLV 基于 HTTP 的长连接特点进行数据的传输，支持HTML5，具有较低的延时\cite{李世满2019以}。
...
因此极大程度的简化了基于Spring框架的应用的搭建和开发\cite{朱桂川0基于神经网络的用电行为监测技术的研究}。
...
Messaging（消息）
和Test（测试）七个大模块\cite{SpringBoot开发实战：微课视频版}。
...
\hline
前置条件&\multicolumn{5}{|l|}{无}\\
\hline
\multirow{4}*{基本事件流}&\multicolumn{5}{|l|}{1. 用户打开系统登陆页面}\\ 
&\multicolumn{5}{|l|}{2. 用户输入用户名、密码和图形验证码}\\
&\multicolumn{5}{|l|}{3. 用户点击登陆按钮}\\
&\multicolumn{5}{|l|}{4. 提示登陆成功，跳转到系统主界面}\\
\hline
其他事件流&\multicolumn{5}{|l|}{在点击登陆之前，用户名、密码、验证码输入框都不能为空}\\
\hline
\multirow{2}*{异常事件流}&\multicolumn{5}{|l|}{1. 提示错误信息}\\
&\multicolumn{5}{|l|}{2. 更新验证码}\\
\hline
后置条件&\multicolumn{5}{|l|}{系统进入主界面，选择记住密码后用户名和密码保存到本地}\\
\hline
...
\hline
前置条件&\multicolumn{5}{|l|}{用户已经登陆，拥有相关菜单权限和数据权限}\\
\hline
\multirow{4}*{基本事件流}&\multicolumn{5}{|l|}{1. 用户进入设备列表页面，可以翻页查看、条件搜索}\\
&\multicolumn{5}{|l|}{2. 用户更改设备删除状态，对设备进行删除和恢复}\\
&\multicolumn{5}{|l|}{3. 用户点击添加按钮，填写设备相关信息后提交完成添加}\\
&\multicolumn{5}{|l|}{4. 用户点击修改按钮，弹出对话框，更改信息后提交修改}\\
\hline
其他事件流&\multicolumn{5}{|l|}{在添加设备、修改设备时，用户填写的数据必须符合要求}\\
\hline
\multirow{2}*{异常事件流}&\multicolumn{5}{|l|}{1. 提示信息填写错误}\\
&\multicolumn{5}{|l|}{2. 用户重新填写后再次提交}\\
\hline
后置条件&\multicolumn{5}{|l|}{无}\\
\hline
\end{longtable}
...
\begin{longtable}[ht]{|p{2.5cm}|p{2cm}|p{2.5cm}|p{2cm}|l|l|}
    ...
    \label{Tab:video}\\
\hline
\textbf{用例名称}&视频监控&\textbf{用例标识号}&Video&\textbf{参与者}&用户\\
\hline
前置条件&\multicolumn{5}{|l|}{用户已经登陆，拥有相关菜单权限和数据权限}\\
\hline
\multirow{5}*{基本事件流}&\multicolumn{5}{|l|}{1. 用户选择录像回放功能，支持搜索和分页查询}\\
&\multicolumn{5}{|l|}{2. 在文件列表中可以选择任意文件进行在线播放和下载}\\
&\multicolumn{5}{|l|}{3. 用户选择实时监控功能，展示设备列表}\\
&\multicolumn{5}{|l|}{4. 设备列表中，未被删除的设备支持实时监控视频的查看}\\
&\multicolumn{5}{|l|}{5. 后台持续进行设备的视频录制}\\
\hline
其他事件流&\multicolumn{5}{|l|}{无}\\
\hline
异常事件流&\multicolumn{5}{|l|}{无}\\
\hline
后置条件&\multicolumn{5}{|l|}{无}\\
\hline
\end{longtable}
...
\begin{longtable}[ht]{|p{2.5cm}|p{2cm}|p{2.5cm}|p{2cm}|l|l|}
    ...
    \label{Tab:info}\\
\hline
\textbf{用例名称}&个人中心&\textbf{用例标识号}&Info&\textbf{参与者}&用户\\
\hline
前置条件&\multicolumn{5}{|l|}{用户已经登陆，拥有个人中心的菜单权限和数据权限}\\
\hline
\multirow{3}*{基本事件流}&\multicolumn{5}{|l|}{1. 用户选择图片进行上传，更改头像}\\
&\multicolumn{5}{|l|}{2. 用户修改用户昵称和密码}\\
&\multicolumn{5}{|l|}{3. 点击提交按钮完成修改}\\
\hline
其他事件流&\multicolumn{5}{|l|}{用户头像、昵称和密码的修改需要符合规范}\\
\hline
\multirow{2}*{异常事件流}&\multicolumn{5}{|l|}{1. 提交无效的数据，提示相关错误信息}\\
&\multicolumn{5}{|l|}{2. 用户修正后重新提交}\\
\hline
后置条件&\multicolumn{5}{|l|}{无}\\
\hline
\end{longtable}
...
\hline
前置条件&\multicolumn{5}{|l|}{用户已经登陆，拥有系统设置的菜单权限和数据权限}\\
\hline
\multirow{6}*{基本事件流}&\multicolumn{5}{|l|}{1. 用户打开系统设置界面}\\
&\multicolumn{5}{|l|}{2. 在用户列表可以对用户进行增删改查}\\
&\multicolumn{5}{|l|}{3. 在角色列表可以进行角色的增删改查}\\
&\multicolumn{5}{|l|}{4. 在权限列表可以进行权限的增删改查}\\
&\multicolumn{5}{|l|}{5. 在角色列表选择角色之后，可以关联权限到该角色上}\\
&\multicolumn{5}{|l|}{6. 在用户列表选择用户之后，可以关联角色到该用户上}\\
\hline
其他事件流&\multicolumn{5}{|l|}{在进行关联、修改数据等操作时，需要符合数据规范}\\
\hline
\multirow{2}*{异常事件流}&\multicolumn{5}{|l|}{1. 提交数据不符合规范，提示错误信息}\\
&\multicolumn{5}{|l|}{2. 用户修正信息后重新提交}\\
\hline
后置条件&\multicolumn{5}{|l|}{无}\\
\hline
\end{longtable}
...
这就是软件六大设计原则之一的依赖倒置原则，即面向接口编程\cite{高松2011敏捷设计原则与设计模式的编程实践——单一职责原则与依赖倒置原则}。
...
该模块提供文件相关的HTTP接口，分别是监控回放文件的列表查询接口和监控回放文件下载接口。
...
该模块负责接收前端的请求，然后调用下层模块（如视频文件存取模块）提供的接口方法，完成业务逻辑，
...
用户单击个人中心可以跳转到个人中心页面；
用户单击退出会弹出二次确认对话框，选择“是”后会退出登陆并跳转到登陆页面。
...
\begin{figure}[ht]
    ...
    \caption{一个导航示例}\label{Fig:eg_main}
\end{figure}
...
\subsubsection{列表}
列表页面展示数据，由后端接口返回。类似的，不同的页面列表部分展示的数据也不尽相同。
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=.8\linewidth]{./Figure/IMG_record_eg.png}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=.8\linewidth]{./Figure/IMG_user.png}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=.8\linewidth]{./Figure/IMG_add_role.png}
...
\begin{longtable}[ht]{|l|l|l|l|l|}
    ...
    \hline
    \multirow{3}*{4}&\multirow{3}*{添加设备}&1. 输入设备名称&\multirow{3}*{提示添加成功}&\multirow{3}*{通过}\\
    &&2. 选择设备类型&&\\
    &&3. 输入设备地址&&\\ 
    \hline
